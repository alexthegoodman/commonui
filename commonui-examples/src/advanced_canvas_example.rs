...
pub fn create_advanced_canvas_app() -> Result<App, Box<dyn std::error::Error>> {
    ...
    let advanced_canvas = canvas()
        ...
        .with_direct_render_func(|device: &Device, queue: &Queue, view: &wgpu::TextureView, _view_width: u32, _view_height: u32, x, y, width, height| {
            // Part 2: Custom vertex rendering with direct GPU commands
            ...
            // Create a command encoder for our custom rendering
            let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("Triangle Render Encoder"),
            });
            
            {
                // Create render pass that renders to the same texture view as Vello
            ...
            // queue.submit run within vello after this function is called
            // queue.submit(std::iter::once(encoder.finish()));
            
            println!("Triangle rendered at position ({}, {}) with size {}x{}", x, y, width, height);
            Ok(())
        });
